...
public class JdbcGameDAO implements GameDAO {
    ...
    static {
        System.out.println("************ jdbc game DAO *****************");
    }
    ...
    public List<Game> getAllGames() {
        ...
        String drawSQL = "SELECT draws.o_draws_id, draws.jackpot, draws.date ,draws.day, draws.vendorCode  FROM o_draws as draws "
                + "JOIN draw_status ON draws.o_draws_id = draw_status.draw_id "
                + "JOIN draw_statuses ON draw_status.status_id = draw_statuses.draw_statuses_id "
                + "WHERE o_game_id=? AND draw_statuses.status_name = 'Active' AND draw_status.deleted = FALSE ;";
...
//				+ "WHERE gameId=? AND draw_statuses.name = 'Active' AND draw_status.deleted = FALSE";
        ...
        List<Map<String, Object>> rows = this.jdbcTemplate.queryForList(gameSQL);
          for (Map row : rows) {
            ...
            List<Map<String, Object>> dRows = this.jdbcTemplate.queryForList(drawSQL, new Object[]{row.get("o_draws_id")});
...
//        }
    ...
    public List<StandardDraw> getDraws() {
        ...
        String drawSQL = "SELECT draws.o_draws_id, "+ 
"draws.jackpot, "+
"draws.date, "+
"draws.day, "+
"draws.vendorCode, "+
"games.game_id AS gameId, "+
"games.game_name AS gameName, "+
"draw_status.status_id as statusId,"+ 
"draw_statuses.status_name AS status "+
"FROM o_draws as draws "+
"JOIN games ON draws.o_game_id = games.game_id "+
"JOIN draw_status ON draw_status.draw_id = draws.o_draws_id"+
"JOIN draw_statuses ON draw_status.status_id = draw_statuses.draw_statuses_id "+
"WHERE draw_status.deleted = FALSE;";
        List<StandardDraw> draws = new ArrayList<StandardDraw>();
