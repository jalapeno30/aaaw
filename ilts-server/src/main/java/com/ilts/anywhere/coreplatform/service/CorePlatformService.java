...
public class CorePlatformService {
	 ...
	 * 
	 */
	public Response processBetAction(final ArrayList<Integer> orderId, String date, Integer userId) {

		if (!cp.isValidSession(userId.toString(), "")) {

			//Invalid Session
			tl.log("Invalid Session");
			tl.logTransaction(userId, "Invalid session", new Object() {
				public ArrayList<Integer> orders = orderId;
			});
