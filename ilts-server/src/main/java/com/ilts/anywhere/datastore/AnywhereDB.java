...
//			purchase.setUserId(((Long)(row.get("userId"))).intValue());
	...
	public List<PurchaseWrapper> getPurchases(int userId) {
	...
	public PaypalPurchase createPaypalPurchase(ArrayList<Integer> orderId, String date, int userId) {
	...
	public ArrayList<PaypalOrder> getPaypalPurchases(ArrayList<Integer> orderId) {
...
//		parameters.addValue("ids", orderId);
		...
		String sql = "SELECT orders.cost AS orderCost, "
				...
				+ "FROM orders "
				+ "JOIN draws ON orders.drawId = draws.drawId "
				+ "JOIN games ON draws.gameId = games.gameId WHERE orders.id = ?";
...
//			System.out.println(link.getHref());
			this.jdbcTemplate.update(sql, new Object[] {
					purchaseId, link.getHref(), link.getRel(), link.getMethod()});
		}
	}
